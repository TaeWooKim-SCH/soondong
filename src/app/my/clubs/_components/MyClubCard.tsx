...
export default function MyClubCard({ myClubInfo }: PropsType) {
  ...
  return (
    <article className="flex w-[300px] border-[1.5px] border-silver rounded-md p-3 sm:w-[600px] sm:p-5">
        <Image
          className="w-[80px] h-[120px] object-cover mr-3 sm:mr-5 sm:w-[160px] sm:h-[240px]"
      ...
      <section className="w-full flex flex-col justify-between">
        ...
        <div className="flex justify-end">
          <button className="text-xs py-1 px-4 border border-blue rounded-md mr-2 sm:text-sm sm:py-2 sm:px-7 sm:mr-5">정보수정</button>
          <button className="text-xs py-1 px-4 border border-blue rounded-md sm:text-sm sm:py-2 sm:px-7">구성원 관리</button>
        </div>
      </section>
    </article>
  );
